...
import { MessageSquare, Send, X, Mail, Phone, MapPin, Briefcase, Award, Building, Paperclip, File, Download } from "lucide-react";
...
export default function SecureChat() {
  ...
  const FileDisplay = ({ fileAttachment, fileType, sender, senderId }) => {
  ...
  const handleViewProfile = async (conversation) => {
    ...
    try {
      // For citizens, we need to fetch the provider profile (lawyer/NGO)
      // The conversation contains the role and we need to find the provider ID from the match
      if (conversation.role === "CITIZEN") {
        // Get match details to find provider info
        const matchesResponse = await getMyAssignedCases(0, 100);
        const matches = matchesResponse.data?.content || [];
        const match = matches.find(m => m.matchId === conversation.id);

        if (match) {
          let profileData;
          if (match.providerType === "LAWYER") {
            const response = await getLawyerProfileById(match.providerId);
            profileData = {
              ...response.data,
              type: "LAWYER",
          ...
          } else if (match.providerType === "NGO") {
            const response = await getNgoProfileById(match.providerId);
            profileData = {
              ...response.data,
              type: "NGO",
              email: conversation.email,
      ...
      } else if (conversation.role === "LAWYER") {
        // Fetch lawyer profile if we have the match
        const matchesResponse = await getMyAssignedCases(0, 100);
        const matches = matchesResponse.data?.content || [];
        const match = matches.find(m => m.matchId === conversation.id);
      ...
      <div className="flex-1 flex flex-col">
        {activeConversation ? (
          <>
            ...
            <div className="bg-white border-t p-4 flex gap-3 items-center">
              <input
                type="file"
                ref={fileInputRef}
